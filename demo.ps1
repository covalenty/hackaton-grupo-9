...
.EXAMPLE
    .\demo.ps1
    Sequencia completa de 4 mensagens texto (3s entre alertas)

.EXAMPLE
    .\demo.ps1 -Interval 5
    5s entre alertas (bom para screen recording)

.EXAMPLE
    .\demo.ps1 -Only 3
    So o cenario 3 (CIENTY_BETTER, Paracetamol Codeina)

.EXAMPLE
    .\demo.ps1 -Image runs\demo_offer.jpg
    Roda extracao de imagem (vision) e dispara os alertas correspondentes.

.EXAMPLE
    .\demo.ps1 -Image runs\demo_offer.jpg -MaxAlerts 3 -Sender "Eduardo MILFARMA"
    Limita a 3 alertas e customiza o nome do rep.
#>
...
if (-not (Test-PythonVenv)) {
    Write-Host "[demo] venv invalido - recriando (uv GC pode ter removido o Python base)..."
    $uv = "$env:LOCALAPPDATA\Microsoft\WinGet\Packages\astral-sh.uv_Microsoft.Winget.Source_8wekyb3d8bbwe\uv.exe"
    if (-not (Test-Path $uv)) { Write-Error "uv nao encontrado. Rode: winget install astral-sh.uv" }
    Push-Location $here
...
if ($PSCmdlet.ParameterSetName -eq 'Image') {
    ...
    $argList = @($Image, "--sender", $Sender, "--send-url", $bridgeSend, "--max-alerts", $MaxAlerts)
    if ($Caption) { $argList += @("--caption", $Caption) }
} else {
    $script = Join-Path $here "scripts\demo_client.py"
    $argList = @("--interval", $Interval)
    if ($Only -gt 0) { $argList += @("--only", $Only) }
}
